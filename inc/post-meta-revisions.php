<?php
/**
 *  Save post meta to revisions.
 *
 *  Code copied from plugin made by Adam Silverstein and
 *  others at https://core.trac.wordpress.org/ticket/20564
 *
...
if ( ! class_exists( 'WP_Post_Meta_Revisioning' ) ) :
	class WP_Post_Meta_Revisioning {
		...
		public static function _wp_post_revision_meta_keys( $only_keys = true ) {
			 ...
			 */
			$keys = apply_filters( 'wp_post_revision_meta_keys', array() );
		...
		public function _wp_preview_meta_filter( $value, $object_id, $meta_key, $single ) {
			...
			$post = get_post();
			if ( empty( $post ) || $post->ID != $object_id || ! in_array( $meta_key, $this->_wp_post_revision_meta_keys() ) || 'revision' == $post->post_type ) {
				return $value;
			}
